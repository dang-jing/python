...
class get_result(object):
    def __init__(self,host,imageDir):
        ...
        self.Secret = "353b6828911ee711f4af650cb97772f0"
        # 接口APIKey（到控制台公式识别服务页面获取）
        self.APIKey= "59b38f1b7ca45f1e81fcfbe25ffb5861"
        ...
        if self.APPID == '' or self.APIKey == '' or self.Secret == '':
            ...
            if status_code!=200:
                ...
                for i in range(len(respData['data']['region'])):
                    res+=respData['data']['region'][i]['recog']['content']
                res=res.replace(" ifly-latex-begin ","").replace(" ifly-latex-end ","").replace("cases","array").replace("cases","array")
                code = str(respData["code"])
                if code!='0':
                    print("请前往https://www.xfyun.cn/document/error-code?code=" + code + "查询解决办法")
                return res
def third(images_dir): 
    image_paths = images_dir
    
    host = "rest-api.xfyun.cn"
    gClass=get_result(host,image_paths)
    res=gClass.call_url()
    return res


    #初始化类
images_dir="D:\\服务器\\itr_teach_python3_demo\\itr\\1.jpg"
res=third(images_dir)
print(res)
