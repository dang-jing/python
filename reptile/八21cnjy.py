...
URL = 'https://www.21cnjy.com/3/79622_7/'
...
class TwoOnecnjy():
    ...
    def set_json(self, title):
        ...
        title = title.split(title[title.rfind('（'):])[0]
        # print(title)
        region = []
        if '省' in title:
        ...
        if '七年级' in title:
            ...
            grade = '九年级'
        if '册' in title:
            self.json['grade'] = grade + title.split('册')[0][-1] + '学期'
        elif '学期' in title:
            a = title.split('学期')[0]
            if '上' in a[-1] or '下' in a[-1]:
                self.json['grade'] = grade + a[-1] + '学期'
            if '）' in title:
                self.json['grade'] = grade + title.split('）')[0][-1] + '学期'
            else:
                self.json['grade'] = grade + a[-2:] + '学期'
        else:
            self.json['grade'] = grade
        if '复习' in title or '训练' in title:
            if '：' in title:
                ...
                q = title.split('《')[1]
                self.json['knowledge'] = q.split('》 ')[0]
            else:
                self.json['knowledge'] = '复习题'
                # label = self.json
        # print(label)
...
TwoOnecnjy()
'''a = '人教版八年级数学上册期末复习单元知识检测 第十五章 分式（Word版 含解析）'
#split = a.split(a[a.rfind('（'):])
a_split = a.split(' ')
print(a_split)'''
